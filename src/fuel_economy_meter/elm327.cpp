...
void ELM327_poll()
{
  ...
  { // OBDRequest came in?
    if (OBDRequest.substring(0, 2) == "AT")
    {                                      // Is it an AT request? used to setup connection between Torque and Arduino
      ...
      if (ATCommand == "DPN")
      {                        // Torque askes for device protocol by number
    ...
    if (OBDRequest.substring(0, 2) == "21")
    ...
    if (OBDRequest.substring(0, 2) == "01")
    {                                       // Mode 1 request
